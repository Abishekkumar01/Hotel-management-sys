...
export async function getServerSideProps(ctx) {
  if (publicRuntimeConfig?.API_BASE_URL) {
    return { props: { room: null, error: { message: 'Backend required. Remove API_BASE_URL for demo.' } } };
  }

  const slug = ctx.query.slug;
  const match = localRooms.find((r) => r.fields.slug === slug || r.sys.id === slug);
  if (!match) {
    return { props: { room: null, error: { message: 'No such room could be found!' } } };
  }

  const data = {
    id: match.sys.id,
    room_name: match.fields.name,
    room_slug: match.fields.slug,
    room_price: match.fields.price,
    room_size: match.fields.size,
    room_capacity: match.fields.capacity,
    allow_pets: match.fields.pets,
    provide_breakfast: match.fields.breakfast,
    room_description: match.fields.description,
    room_status: 'available',
    room_images: (match.fields.images || []).map((img) => ({ url: img?.fields?.file?.url }))
  };

  return { props: { room: { data }, error: null } };
}
